...
const Start = async () => {
    ...
    await schema.CreateChildNode(1, "Ruslan Khazbulatov", "# Links\n [VK](https://vk.com/almostz)")
    await schema.CreateChildNode(1, "Roman Armansky", "# Links\n [VK](https://vk.com/h4rdc0r33)")
    await schema.CreateChildNode(1, "Alexandra Rudenko", "# Links\n [VK](paste link here)")
    await schema.CreateChildNode(1, "Arman Davletov", "# Links\n [VK](paste link here)")
    ...
    render() {
        return (
            <div className="App">
                ...
                <button onClick={()=>{schema.FindChildren(1).then(d=>{console.log(d)})}}>Log the data</button>
            </div>
        )
    }
}
